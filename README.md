...
- `my_htons()` — done, verified by hand against `htons` on test values
- `my_htonl()` — done, verified by hand against `htonl` on test values
- Test suite (`tests/test_netutils.c`) comparing against glibc originals — not yet written
- Next: `my_inet_pton()`
